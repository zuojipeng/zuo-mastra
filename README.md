...
`usable` 镜头只有在当前选中尝试具备匹配的 `human_approval` 回执时才会进入可交接状态。回执必须绑定相同镜头、尝试、provider、model 和素材引用，并带有效审批时间与非空决策说明。旧版或格式异常的 payload 仍可读取，但会以 `镜头 N 缺交付审批` 失败关闭，直到用户重新完成交付审批。该回执仅代表应用内人工复核证据，不构成身份、法律或密码学证明。
