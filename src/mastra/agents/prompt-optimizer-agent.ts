...
/**
 * ========================================
 * AI 智能超级提示词优化 Agent (MVP 版本)
 * ========================================
 * 
 * 功能说明：
 * 这个 Agent 的主要作用是帮助用户优化他们的提示词，使其更适合各种 AI 工具理解和处理。
 * 
 * 核心能力：
 * 1. 分析用户输入的原始问题/提示词
 * 2. 识别提示词中的不清晰、模糊或不完整的部分
 * 3. 根据最佳实践，重构并优化提示词
 * 4. 提供优化后的多个版本（适用于不同场景）
 * 5. 给出优化建议和解释
 * 
 * MVP 版本特点：
 * - 纯 LLM 驱动，无需外部工具
...
export const promptOptimizerAgent = new Agent({
  // Agent 名称：用于在 Mastra 平台中识别和管理
  name: 'Prompt Optimizer Agent',
  
  /**
   * 系统指令：定义 Agent 的行为规范和工作方式
   * 这是 Agent 的"人格设定"和"工作手册"
   */
  instructions: `
...
`,
  ...
  /**
   * 模型配置：选择使用的 LLM 模型
   * gpt-4o-mini：性价比高，响应快，适合 MVP 版本
   * 后续可升级为 gpt-4o 或 claude-3.5-sonnet 以获得更好的分析能力
   */
